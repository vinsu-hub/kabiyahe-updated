...
import { Header, Footer, BottomNav, RightRailCard, PassportPromoCard, ListingCard, notify } from "@/components/shell";
...
export function Explore() {
  const { data: allDestinations, isLoading, error, refetch } = useDestinations();
  ...
  const filtered = useMemo(() => (allDestinations ?? []).filter(d =>
    !d.placeholder && (!category || d.type === category) && (!tag || d.tags.includes(tag)) && d.price_tier <= price && `${d.name} ${d.place ?? ""} ${d.type} ${d.tags.join(" ")}`.toLowerCase().includes(query.trim().toLowerCase())
    ...
    <div className="explore-v2-layout">
      <div className="explore-v2-main">
        <div className="explore-v2-lead">
          ...
          {showFilters && <div className="explore-v2-extra-filters"><label>Tag <select value={tag} onChange={e => setTag(e.target.value)}><option value="">All tags</option>{Array.from(new Set((allDestinations ?? []).flatMap(d => d.tags))).sort().map(t => <option key={t}>{t}</option>)}</select></label><label>Max. price <input type="range" min="1" max="4" value={price} onChange={e => setPrice(Number(e.target.value))}/> {"₱".repeat(price)}</label><label>Sort <select value={sort} onChange={e => setSort(e.target.value as typeof sort)}><option>Popular</option><option>Name</option>{loc.coords && <option>Nearby</option>}</select></label><a href="https://www.airbnb.com/s/Los-Banos--Laguna--Philippines/homes" target="_blank" rel="noreferrer">Find Los Baños stays on Airbnb <ExternalLink size={13}/></a></div>}
          <div className="explore-v2-chips" role="group" aria-label="Browse categories">{groups.map(g => <button key={g.label} className={category === g.type ? "active" : ""} onClick={() => choose(g.type)}><g.icon size={22}/><span>{g.label}</span></button>)}</div>
        </div>
