...
import { BusTours, Delicacies, EventDetail, EventsList, Parking, Passport, RideGuide, StayEat, TourDetail } from "@/pages/ElbiyaheFeatures";
...
function Home() {
  ...
  return <><Header/><main className="landing-page elbiyahe-home">
    ...
    <section className="container explore-preview scroll-reveal" data-scroll-reveal>
      ...
      <div className="mini-map"><div className="map-water"/><div className="map-label one">Calamba City</div><div className="map-label two">Laguna<br/>de Bay</div>{(destinations ?? []).slice(0,5).map((d,i)=><Link href={`/explore/${d.slug}`} className={`pin ${["nature","food","culture","stay","gem"][i]}`} style={{left:`${18+i*16}%`,top:`${28+(i%2)*27}%`}} key={d.id}>⌖</Link>)}</div>
    </section>
